<script lang="ts">
  ...
  import { SEO_PAGES, breadcrumbJsonLd, courseJsonLd, webPageJsonLd } from '$lib/seo';
    ...
    (async () => {
      try {
        ...
        if (weakRes.ok) {
          ...
          weakKeys = (w.weakKeys ?? [])
            .slice(0, 5)
            .map((k: { key: string }) => k.key.toUpperCase());
  ...
  function dismissBanner() {
    ...
    try {
      sessionStorage.setItem('tf_adaptive_banner_dismissed', '1');
    } catch { /* ignore */ }
  }
  ...
  const seo = SEO_PAGES.learn;
  const learnJsonLd = [
    webPageJsonLd({ ...seo }),
    courseJsonLd({
      name: 'TypingScholar Structured Typing Lessons',
  ...
  onMount(() => {
    ...
    const initialLanguage =
      storedLanguage && lessonLanguageCodes.has(storedLanguage) ? storedLanguage : localeLanguage;
  ...
  $effect(() => {
    ...
    if (
      ...
      !lessonLanguageCodes.has(localeLanguage) ||
      localeLanguage === lastSyncedLocale
    )
      return;
  ...
  function setCourseLayout(layoutId: string) {
    userLayout = layoutId;
    try {
      localStorage.setItem(LS_LAYOUT, layoutId);
    } catch { /* ignore */ }
  }
  ...
  function toLessonCardProps(lesson: Lesson) {
    ...
    return {
      id: lesson.id,
      title: lesson.title,
      description: `Focus Characters: ${
        lesson.tags?.key_bigram ||
        lesson.content
          .slice(0, 15)
          .map((c: any) => c.char)
          .join('')
  ...
  const completedForTrack = $derived(
    filteredLessons.filter((l) => completedLessonIds.has(l.id)).length
  );
  const trackProgressPercent = $derived(
    filteredLessons.length > 0 ? Math.round((completedForTrack / filteredLessons.length) * 100) : 0
  );
...
<SeoHead title={seo.title} description={seo.description} path={seo.path} jsonLd={learnJsonLd} />
...
<div class="flex flex-col lg:flex-row min-h-[calc(100vh-80px)] w-full relative">
  <SideNavBar title={$t('learn_course_settings')}>
    ...
    <div class="flex flex-col gap-1.5 w-full mt-4">
      <label
        for="lang-select"
        class="text-[0.65rem] font-bold text-on-surface-variant uppercase tracking-widest"
        >{$t('learn_target_language')}</label
      >
      <select
        id="lang-select"
  ...
  <div class="flex-1 w-full max-w-6xl mx-auto px-4 md:px-8 py-8 lg:py-12">
    ...
    <div
      class="mb-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-6 p-8 bg-gradient-to-br from-surface-container-low to-surface-container-lowest shadow-xl border border-surface-container border-b-4 border-b-primary relative overflow-hidden"
    >
      <div class="absolute right-0 top-0 opacity-5 pointer-events-none w-64 h-64 -mt-10 -mr-10">
        <svg
          ...
          ><path d="M12 15l-3.5-3.5" /><path d="M15.5 11.5L12 8" /><path d="M6 18h12" /><path
            d="M7 21h10"
          /><path
            d="M3.5 12.5v-7A1.5 1.5 0 0 1 5 4h14a1.5 1.5 0 0 1 1.5 1.5v7C20.5 17 12 21 12 21s-8.5-4-8.5-8.5z"
          /></svg
        >
      </div>
      <div class="relative z-10 max-w-xl">
        ...
        <div class="w-full">
          <div class="flex justify-between items-end mb-2">
            <span
              class="font-label text-xs uppercase tracking-widest text-on-surface-variant font-bold"
              >{$t('learn_course_progression')}</span
            >
            <span class="font-headline text-lg text-primary">{trackProgressPercent}%</span>
          </div>
          <div
            class="w-full bg-surface-container-high h-2 rounded-full overflow-hidden shadow-inner"
          >
            <div
          ...
          <div
            class="mt-2 text-[0.65rem] text-on-surface-variant uppercase tracking-widest font-bold"
          >
            {$t('learn_modules_conquered', {
              completed: completedForTrack,
      ...
      <div class="adaptive-banner" role="alert" aria-label="Adaptive drill recommendation">
        <div class="adaptive-banner-left">
          ...
          <div>
            ...
            <p class="adaptive-body">
              ...
              {#each weakKeys as k, i}<span class="key-chip">{k}</span
                >{#if i < weakKeys.length - 1}{/if}{/each}
            </p>
          </div>
        </div>
        <div class="adaptive-banner-actions">
          <a href="/practice?mode=adaptive&weakKeys={weakKeys.join(',')}" class="drill-btn">
            ...
            <span
              class="ml-2 px-1.5 py-0.5 text-[10px] uppercase font-bold tracking-wider bg-amber-500/20 text-amber-500 rounded-sm"
              >{$t('learn_pro_badge')}</span
            >
          </a>
          <button
      ...
      <section class="mb-12">
        <div class="flex items-center gap-3 mb-6">
          ...
          {#if getLanguageByCode(userLanguage)?.nativeName}
            <span
              class="bg-primary/20 text-primary text-xs px-2 py-1 rounded shadow-sm font-label uppercase tracking-widest"
              >{getLanguageByCode(userLanguage)?.nativeName}</span
            >
          {/if}
        </div>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {#each recommendedLessons as lesson}
            <LessonCard lesson={toLessonCardProps(lesson)} href={`/learn/${lesson.id}`}>
              <div class="flex flex-wrap gap-2 mt-2">
                {#each getLessonTags(lesson).slice(0, 3) as tag}
                  <span
                    class="text-xs px-2 py-0.5 bg-surface-container text-on-surface-variant rounded-sm"
                  >
                    {tag}
                  </span>
                {/each}
    ...
    <section>
      ...
      {#if filteredLessons.length > 0}
        <div class="space-y-16">
          ...
          {#if selectedLanguage === userLanguage && selectedDifficulty === 'all' && selectedTag === 'all' && !searchQuery}
            <div
              class="relative progressive-path pl-4 md:pl-8 border-l-2 border-surface-container-highest"
            >
              {#each groupedLessons as stage, index}
                <div class="mb-14 relative">
                  <!-- Timeline node marker -->
                  <div
                    class="absolute -left-[21px] md:-left-[37px] top-4 w-10 h-10 rounded-full bg-surface-container flex items-center justify-center border-4 border-background text-primary font-bold z-10 shadow-[0_0_15px_rgba(240,165,0,0.2)]"
                  >
                    {index + 1}
                  </div>

                  <div
                    class="mb-6 ml-6 p-6 bg-surface-container-low/50 backdrop-blur-sm rounded-xl border border-surface-container shadow-sm flex items-center justify-between"
                  >
                    <div>
                      <h3 class="font-headline text-2xl text-on-surface">
                        {$t('learn_stage_label', { level: stage.level })}: {stage.title}
                      </h3>
                  ...
                  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 ml-6">
                    {#each stage.lessons as lesson}
                      ...
                      {#if lesson.isTest}
                        <div
                          class="md:col-span-2 lg:col-span-3 w-full group relative transition-transform {isLocked
                            ? 'opacity-50 grayscale cursor-not-allowed'
                            : 'hover:-translate-y-1'}"
                        >
                          <div
                            class="absolute inset-0 bg-primary/5 rounded-2xl border-2 {isLocked
                              ? 'border-surface-variant'
                              : 'border-primary/50 group-hover:border-primary shadow-[0_0_25px_rgba(240,165,0,0.15)]'} transition-all"
                          ></div>
                          <div
                            class="relative p-6 flex flex-col md:flex-row items-center justify-between gap-6"
                          >
                            <div class="flex-1 flex items-start gap-4">
                              <div
                                class="{isLocked
                                  ? 'bg-surface-variant text-surface-container'
                              ...
                              >
                                ...
                                {:else}
                                  <svg
                                    ...
                                    stroke-linecap="round"
                                    stroke-linejoin="round"
                                    ><path d="M12 15l-3.5-3.5" /><path d="M15.5 11.5L12 8" /><path
                                      d="M6 18h12"
                              ...
                              <div>
                                <h4
                                  class="font-headline text-xl text-primary font-bold uppercase tracking-widest"
                                >
                                  {lesson.title}
                                </h4>
                                <p class="text-on-surface-variant text-sm mt-1">
                                  {$t('learn_unlock_test_desc')}
                                </p>
                                <div class="flex gap-4 mt-3">
                                  <span
                                    class="bg-primary/10 text-primary text-xs font-bold px-2 py-1 rounded"
                                    >{$t('learn_min_accuracy')}</span
                                  >
                                  <span
                                    class="bg-surface-container text-on-surface-variant text-xs px-2 py-1 rounded"
                                    >{$t('learn_words_count', {
                            ...
                            <div class="flex flex-col items-center gap-2">
                              {#if completedLessonIds.has(lesson.id)}
                                <div
                                  class="text-primary text-sm font-bold uppercase tracking-widest bg-primary/20 px-4 py-2 rounded-lg border border-primary/30 flex items-center gap-2"
                                >
                                ...
                                {#if lesson.id.includes('-test-5')}
                                  <a
                                    href="/certificate"
                                    class="text-xs hover:text-primary transition-colors underline opacity-70 mt-1 cursor-pointer"
                                    >{$t('learn_view_certification')}</a
                                  >
                                {/if}
                              {:else if isLocked}
                                <button
                                  disabled
                                  class="bg-surface-variant text-on-surface-variant font-bold px-8 py-3 rounded uppercase tracking-wider text-sm cursor-not-allowed"
                                  >{$t('learn_locked')}</button
                                >
                              {:else}
                                <a
                                  href={`/learn/${lesson.id}`}
                                  class="bg-primary hover:bg-opacity-90 text-background font-bold px-8 py-3 rounded uppercase tracking-wider text-sm transition-all focus:ring-2 focus:ring-primary focus:ring-offset-2 focus:ring-offset-background outline-none hover:shadow-[0_0_15px_rgba(240,165,0,0.5)]"
                                  >{$t('learn_start_test')}</a
                                >
                              {/if}
                            </div>
                          </div>
                        ...
                        >
                          ...
                          <div class="flex items-center gap-2 mt-3 text-amber-400 text-sm">
                            ...
                            <span class="text-on-surface-variant text-xs"
                              >({Math.round(lesson.content.length)} chars)</span
                            >
                          </div>
                        </LessonCard>
                      {/if}
              ...
              {#each filteredLessons as lesson}
                <LessonCard lesson={toLessonCardProps(lesson)} href={`/learn/${lesson.id}`}>
                  ...
                  <div class="flex items-center gap-2 mt-3 text-amber-400 text-sm">
                    <span>{getDifficultyStars(lesson.difficulty)}</span>
                    <span class="text-on-surface-variant text-xs"
                      >({lesson.content.length} chars)</span
                    >
                  </div>
                </LessonCard>
              {/each}
...
<style>
  ...
  .adaptive-banner {
    ...
    background: linear-gradient(
      135deg,
      rgba(255, 197, 108, 0.1) 0%,
      rgba(255, 197, 108, 0.04) 100%
    );
