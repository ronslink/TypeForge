<script lang="ts">
  ...
  <div class="h-[calc(100vh-80px)] w-full max-w-screen-xl mx-auto flex flex-col p-4 md:p-8">
    <div class="mb-6 flex items-center justify-between">
      <div>
        <h1 class="text-3xl font-black uppercase tracking-tighter text-on-surface font-headline">{$t('cascade_title')}</h1>
    ...
    <div class="flex-1 flex flex-col items-center justify-center relative">
      {#if !isPlaying && !showResults}
        <div class="w-full max-w-md bg-surface-container-low p-8 border border-outline-variant/30 rounded-2xl shadow-xl flex flex-col gap-6 text-center">
          ...
          <div>
            <h2 class="text-2xl font-black font-headline text-on-surface mb-2">{$t('cascade_init_title')}</h2>
            <p class="text-sm text-on-surface-variant leading-relaxed">{$t('cascade_init_desc')}</p>
          </div>
  
          <div class="flex flex-col gap-2 text-left">
            <label for="lang" class="text-xs uppercase tracking-widest font-bold text-on-surface-variant ml-1">{$t('cascade_dictionaries')}</label>
      ...
      {#if showResults}
        <div class="w-full max-w-md bg-surface-container-low p-8 border border-outline-variant/30 rounded-2xl shadow-xl flex flex-col gap-6 text-center animate-in fade-in slide-in-from-bottom-8">
           <h2 class="text-4xl font-black text-primary font-headline uppercase tracking-tighter">{$t('cascade_breach')}</h2>
           
           <div class="grid grid-cols-2 gap-4 my-2">
              <div class="bg-surface-container p-4 rounded-xl border border-outline-variant/20 shadow-inner">
                <div class="text-xs uppercase font-bold text-on-surface-variant tracking-wider mb-1">{$t('cascade_score')}</div>
                <div class="text-3xl font-black text-on-surface">{finalScore}</div>
              </div>
              <div class="bg-surface-container p-4 rounded-xl border border-outline-variant/20 shadow-inner">
                <div class="text-xs uppercase font-bold text-on-surface-variant tracking-wider mb-1">{$t('cascade_accuracy')}</div>
