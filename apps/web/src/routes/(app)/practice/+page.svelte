<script lang="ts">
  ...
  const canStop = $derived(
    isStarted && !isComplete && keystrokes.filter((k) => k.correct).length >= 10
  );
...
<SeoHead title={seo.title} description={seo.description} path={seo.path} jsonLd={practiceJsonLd} />
...
<div class="flex flex-col lg:flex-row min-h-[calc(100vh-80px)] w-full relative">
  <SideNavBar title={$t('nav_practice') || 'Practice Mode'}>
    ...
    <div class="flex flex-col gap-1.5 w-full">
      <label
        for="practice-layout"
        class="text-[0.65rem] font-bold text-on-surface-variant uppercase tracking-widest"
        >{$t('practice_layout') || 'Keyboard Layout'}</label
      >
      <select
        id="practice-layout"
        class="bg-surface-container/50 px-4 py-2.5 text-sm font-medium text-on-surface focus:outline-none focus:border-primary focus:bg-surface-container border border-transparent shadow-inner cursor-pointer transition-colors w-full"
  ...
  <div class="flex-1 w-full max-w-6xl mx-auto px-4 md:px-8 py-8 lg:py-12">
    {#if mode === 'select'}
      <h1 class="font-headline text-4xl mb-4 text-primary">
        {$t('practice_sandbox_title') || 'Practice Sandbox'}
      </h1>
      <p class="text-on-surface-variant mb-12 max-w-2xl text-sm">
        {$t('practice_sandbox_desc') ||
          'Freeform typing simulation. Submitting tracks grants active backend XP profiles without unlocking Curriculum states. Select your drill sequence below.'}
      </p>

      <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
        <button
          onclick={() => mountPracticeMode('words')}
          class="bg-surface-container-low p-6 rounded-2xl border border-transparent text-left hover:border-primary hover:-translate-y-1 transition-all flex flex-col gap-3"
        >
          <h3 class="font-headline text-xl mb-2">{$t('practice_mode_words') || 'Words'}</h3>
          <p class="text-on-surface-variant text-sm flex-1">
            {$t('practice_mode_words_desc') ||
              'Practice common words pulled natively from localized arrays'}
          </p>
        </button>

        <button
          onclick={() => mountPracticeMode('sentences')}
          class="bg-surface-container-low p-6 rounded-2xl border border-transparent text-left hover:border-primary hover:-translate-y-1 transition-all flex flex-col gap-3"
        >
          <h3 class="font-headline text-xl mb-2">{$t('practice_mode_sentences') || 'Sentences'}</h3>
          <p class="text-on-surface-variant text-sm flex-1">
            {$t('practice_mode_sentences_desc') || 'Synthesized punctuated sentence blocks'}
          </p>
        </button>

        <div
          class="bg-surface-container-low p-6 rounded-2xl border border-transparent flex flex-col gap-3"
        >
          <div class="flex items-start justify-between gap-2">
            <h3 class="font-headline text-xl">{$t('practice_mode_literature') || 'Literature'}</h3>
            {#if currentBooks.length > 0}
              <span
                class="text-xs font-label uppercase tracking-widest text-primary bg-primary/10 border border-primary/20 px-2 py-0.5 rounded-full shrink-0"
              >
          ...
          <p class="text-on-surface-variant text-sm flex-1">
            {$t('practice_mode_literature_desc') ||
              'Type passages from public-domain classics in your chosen language.'}
          </p>

          {#if currentBooks.length === 0}
            <p
              class="text-xs text-on-surface-variant italic border border-outline-variant/30 rounded p-3"
            >
    ...
    {:else if isGeneratingAI}
      <div class="flex flex-col items-center justify-center py-24 space-y-6">
        ...
        <h2 class="font-headline text-2xl text-primary animate-pulse">
          {$t('practice_generating_ai')}
        </h2>
    ...
    {:else}
      ...
      {#if isComplete}
        <div
          class="bg-surface-container-low p-8 max-w-md mx-auto text-center rounded-2xl shadow-xl border border-primary/20"
        >
          ...
          <div class="grid grid-cols-2 gap-4 mb-8">
            <div class="bg-surface-container p-4 rounded text-center">
              <span class="block font-label text-sm text-on-surface-variant mb-1"
                >{$t('lesson_speed') || 'Speed'}</span
              >
              <span class="font-headline text-4xl text-on-surface"
                >{finalWPM}
                <span class="text-base text-primary">{$t('lesson_wpm') || 'WPM'}</span></span
              >
            </div>
            <div class="bg-surface-container p-4 rounded text-center">
              <span class="block font-label text-sm text-on-surface-variant mb-1"
                >{$t('lesson_accuracy') || 'Accuracy'}</span
              >
              <span class="font-headline text-4xl text-on-surface"
                >{finalAccuracy}<span class="text-base text-secondary">%</span></span
              >
            </div>
          </div>
          ...
          <div class="flex gap-4 w-full">
            <button
              ...
              class="flex-1 bg-surface-container hover:bg-surface-container-highest text-white font-bold py-3 rounded transition-colors uppercase tracking-wider text-sm font-label"
              >{$t('practice_done') || 'Done'}</button
            >
            <button
              onclick={() => mountPracticeMode(mode as any)}
              class="flex-1 bg-primary hover:bg-opacity-80 text-background font-bold py-3 rounded transition-colors uppercase tracking-wider text-sm font-label"
              >{$t('lesson_retry') || 'Retry Mode'}</button
            >
          </div>
        </div>
      {:else}
        <div class="w-full max-w-4xl mx-auto flex flex-col gap-8 mb-12">
          <MetricsBar
            metrics={[
              ...
              {
                label: $t('lesson_accuracy') || 'Accuracy',
                value: `${currentAccuracy}%`,
                variant: 'secondary',
              },
              {
                label: $t('progress_streak') || 'Streak',
                value: currentStreak,
                variant: 'default',
              },
              {
                label: $t('practice_active_time'),
                value: formatTime(activeElapsedSeconds),
                variant: 'default',
              },
              {
                label: $t('practice_errors') || 'Errors',
                value: keystrokes.filter((k) => !k.correct).length,
                variant: 'default',
              },
            ]}
            {currentStreak}
          ...
          {#if isStarted && !isComplete}
            <div class="flex items-center justify-between -mt-4 px-1" aria-live="polite">
              <div
                class="flex items-center gap-2 text-xs font-label uppercase tracking-widest
...
<style>
  ...
  .pr-pause-dot {
    background: #f59e0b;
    animation: pr-pulse-amber 1.4s ease-in-out infinite;
  }
  .pr-active-dot {
    background: #4ade80;
    animation: pr-pulse-green 2s ease-in-out infinite;
  }
